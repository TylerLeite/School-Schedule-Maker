...
public class Output {
	...
	protected static String format(String[][] schedule, String name){
		...
		final int TIMES = 10, DAYS = 5;
		final int SNACK = 3, LUNCH = 6;
		...
		for (int i = 0, Q = 0; Q < TIMES; i++, Q++){
			 ...
			 * and lunch on the printed schedule but not in the array used 
			 * in the scheduling algorithm. I keep a value Q separate from
			 * i to track how many times total the loop has run and use i
			 * to access the correct cell in schedule[][]. Q is used to 
			 * check whether a snack / lunch row should be added. */
			if (Q == SNACK || Q == LUNCH){
				i--;
			}
		}
