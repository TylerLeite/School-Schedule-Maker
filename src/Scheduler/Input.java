...
public class Input {
    ...
    public static void parseCrsFile(String fileName){
        ...
        if (!Scheduler.teachers.containsKey(teacherName))
            Scheduler.teachers.put(teacherName, new Teacher(teacherName));
        Scheduler.teachers.get(teacherName).addCourseToSchedule(course.name, course.freq);
        ...
        for (String student : studentNames){
            if (!Scheduler.students.containsKey(student))
                Scheduler.students.put(student, new Student(student));
            course.addStudent(student);
            Scheduler.students.get(student).addCourseToSchedule(course.freq);
        }
    }
    ...
    static public void parseTchFile(String fileName) {
        ...
        for (int i = 0; i < openingData.length; i++){
            ...
            Scheduler.teachers.get(name).sch.restrict(day, time);
            Scheduler.teachers.get(name).sch.schedule[day][time] = "DNF";
        }
    }
}